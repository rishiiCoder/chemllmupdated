...
def predict_electronproperties(smiles: str):
   ...
   Returns:
    ...
    print("COMPLETED PREDICT_ELECTRONPROPERTIES")
    return homo * 27.2114
...
def run_molecule_redesign(apikey: str, pdf_path: str):
    ...
    You are computational molecular designer specialized in photochemistry, structure-property relationships, and solar material design.
        ...
        - The 'predict_dft' tool, which predicts various density-functional theories -> prints data related to DFT properties and prints nothing
        - The 'predict_electronproperties' tool, which predicts various electron acception and electron donation properties ->  prints out electron property data and returns nothing
        ...
        6) ONLY IF INVALID MOLECULE -> RETURN to STEP 1 to alter the molecule some more to improve lambda_max value
        7) RIGHT AFTER predict_dft is called -> ATTEMPT TO ANALYZE PRINT STATEMENTS BY IT FOR THE EV-ENERGY LEVELS OF THE MOLECULE
        8) IF predict_dft IS NOT analyzed or IF predict_dft IS analyzed, still IMMEDIETLY -> call predict_electronproperties tool to predict properties related to electron transfers
        9) IF predict_electronproperties has been called: All properties have been predicted for one modified molecule, move onto next and new structure (return to Step 1) and if 
        {TARGET_MOLECULE_COUNT} has NOT been reached
        10) Stop once {TARGET_MOLECULE_COUNT} valid structures are proposed
        ...
        - COMPLETE ALL STEPS for EACH molecule: Use ALL tools in order: validate_mol -> predict_dft -> predict_electronproperties
        ...
        try:
            ...
            # text instead of function call
                ...
                # otherwise, prompt to continue
                    ...
                    reminder = (
                        f"You must generate a molecule and call next property prediction function: validate_mol -> predict_dft -> predict_electronproperties. "
