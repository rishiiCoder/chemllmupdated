...
def predict_lambda(smiles: str):
  ...
  """
  url = "https://yang-requesting-eagle-villages.trycloudflare.com/predict_lambda"
    ...
    def get_feedback_message(self, function_name: str, smiles: str, result) -> str:
        ...
        if function_name == "validate_mol":
            ...
            if lambda_max >= self.threshold:
                ...
                msg = (
                    f"✓✓ ACCEPTED: {smiles}\n"
                    f"   λ_max = {lambda_max:.1f} nm (≥ {self.threshold} nm threshold)\n"
                    f"   Progress: {len(self.valid_molecules)}/{self.target_count} molecules\n"
                )
                if len(self.valid_molecules) >= self.target_count:
                    msg += f"\n TARGET REACHED! You have found {self.target_count} valid molecules.\n"
                    msg += "Now provide your final summary in the requested format."
                else:
                ...
                return (
                    ...
                    f"   λ_max = {lambda_max:.1f} nm (< {self.threshold} nm threshold)\n"
                    f"   Progress: {len(self.valid_molecules)}/{self.target_count} molecules\n"
                    f"   Generate a NEW molecule with different modifications."
                )
    ...
    def print_summary(self):
        ...
        if self.rejected_molecules:
            ...
            for smiles, lambda_max in self.rejected_molecules.items():
                print(f"  - {smiles} (λ_max = {lambda_max:.1f} nm)")
